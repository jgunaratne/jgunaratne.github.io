...
function workOnItem(item, workerN) {
  if (item) {
    ...
    item.li.animate({
      ...
      step : function(now, fx) {
        ...
        if (item.time > 5000) {
          ...
          item.li.parents('li').find('.status').removeClass('down').addClass(
              'up');
        }
      ...
      complete : function() {
        ...
        item.li.parents('li').find('.status').removeClass('down').removeClass(
            'up');
        cText.text('Synced recently');
        setTimeout(function() {
          cText.text('Synced 1 min ago');
        }, 10000);
        setTimeout(function() {
          cText.text('Synced 2 min ago');
        }, 2 * 60000);
        doneStack.push(item);
        $('.file-count-over, .file-count-under').text(
            doneStack.length + ' of ' + n + ' items');
    ...
    item.li.parents('li').find('.icon.syncing').addClass('sync');

    setTimeout(function() {
      item.done = true;
      item.working = false;
      var syncIcon = item.li.parents('li').find('.icon.syncing');
        ...
        $('.activity-container').animate({
          ...
          complete : function() {
            setTimeout(function() {
              ...
              $('.menu-bottom .status').text('Synced');
...
function populateItems() {
  ...
  for ( var i = 0; i < n; i++) {
    ...
    var html = '<li><div class="file-icon"></div><div class="title">'
      ...
      <div class="status down"><div class="load-arrow-container"><div class="load-arrow"></div></div><div class="text-complete">Waiting to sync</div></div>\
      <div class="icon syncing"></div>\
